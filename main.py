...
#Function 
def pred_status(ctry, itmtp,aplcn,wth,prdrf,qtlg,cstlg,tknslg,slgplg,itmdt,itmmn,itmyr,deldtdy,deldtmn,deldtyr):
    itdd = int(itmdt)
    itdm = int(itmmn)
    itdy = int(itmyr)
    ...
    with open("E:/Projects/Industrial Copper Modeling/Classification_model.pkl","rb") as f:
...
def pred_selling_price(ctry,sts,itmtp,aplcn,wth,prdrf,qtlg,cstlg,tknslg,
                       itmdt,itmmn,itmyr,deldtdy,deldtmn,deldtyr):
    itdd = int(itmdt)
    itdm = int(itmmn)
    itdy = int(itmyr)
    ...
    with open("E:/Projects/Industrial Copper Modeling/Regression_Model.pkl","rb") as f:
...
with st.sidebar:
    option = option_menu("Suhail",options=["PREDICT SELLING PRICE","PREDICT STATUS"])
...
if option == "PREDICT STATUS":
    ...
    if button:
        status = pred_status (country,item_type,application,width,product_ref,quantity_tons_log,
...
if option == "PREDICT SELLING PRICE":
    ...
    if button:
        price= pred_selling_price(country,status,item_type,application,width,product_ref,quantity_tons_log,
